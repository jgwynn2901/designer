...
Response.Buffer = true
	...
	If Request.QueryString("TABLENAME") <> "" Then
		SQLST = SQLST & "SELECT DISTINCT COLUMN_NAME FROM user_tab_columns WHERE TABLE_NAME='" & Request.QueryString("TABLENAME") & "'"
	End If
...
<TD CLASS=LABEL><FONT COLOR="WHITE">Column Names for <%= Request.QueryString("TABLENAME") %></FONT></TD>
</TR>
<% Do While Not RS.EOF %>
<TR CLASS=RESULTROW OnClick="Javascript:multiselect(this);" OnDblClick="Javascript:dblhighlight(this);" TABLE='<%= RS("COLUMN_NAME") %>'  title="Click copy button to place name in clipboard.">
<TD CLASS=LABEL><%= RS("COLUMN_NAME") %></TD>
</TR>
<%
RS.MoveNext
Loop
