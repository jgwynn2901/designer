...
<%
	...
	If HasModifyPrivilege("FNSD_LOOKUP_TYPES",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_LOOKUP_TYPES",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
%>
...
{
	LookupTypeSearchObj.LUTID = document.frames("TabFrame").document.frames.GetLUTID();
	LookupTypeSearchObj.LUTIDName = document.frames("TabFrame").document.frames.GetLUTIDName();
}

function IsCurrentSelectionValid()
{
	var LUTID = document.frames("TabFrame").document.frames.GetLUTID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	LookupTypeSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "LookupTypeSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "LookupTypeDetails-f.asp?MODE=<%=MODE%>&LUTID=<%=Request.QueryString("LUTID")%>",1);
<%	end if %>

<%	If (Request.QueryString("LUTID") = "NEW" Or Request.QueryString("LUTID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "LookupTypeDetails-f.asp?MODE=<%=MODE%>&LUTID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				UpdateCurrentObjFromScreen();
				LookupTypeSearchObj.Selected = true;
				window.close();
			}
		}
	}
