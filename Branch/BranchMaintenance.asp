...
{
	BranchSearchObj.BID = document.frames("TabFrame").document.frames.GetBID();
	BranchSearchObj.BIDOfficeName = document.frames("TabFrame").document.frames.GetBIDOfficeName();
	BranchSearchObj.BNUM = document.frames("TabFrame").document.frames.GetBNUM();
}

function IsCurrentSelectionValid()
{
	var BID = document.frames("TabFrame").document.frames.GetBID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	BranchSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %> 
	AddTab("Search",120, "BranchSearch-f.asp?BranchTypeFilter=<%= Request.QueryString("BranchTypeFilter") %>&MODE=<%=MODE%>&SearchAHSID=<%= Request.QueryString("SearchAHSID") %>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "BranchDetails-f.asp?BranchTypeFilter=<%= Request.QueryString("BranchTypeFilter") %>&MODE=<%=MODE%>&BID=<%=Request.QueryString("BID")%>",1);
<%	end if %>

<%	If (Request.QueryString("BID") = "NEW" Or Request.QueryString("BID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "BranchDetails-f.asp?BranchTypeFilter=<%= Request.QueryString("BranchTypeFilter") %>&MODE=<%=MODE%>&BID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
