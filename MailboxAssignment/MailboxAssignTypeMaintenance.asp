<%
'***************************************************************
'Mailbox Assignment Types search and details forms.
'
'$History: MailboxAssignTypeMaintenance.asp $ 
'* 
'* *****************  Version 1  *****************
...
<%
	...
	If HasModifyPrivilege("FNSD_MAILBOX_ASSIGNMENT",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_MAILBOX_ASSIGNMENT",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
%>
...
{
	MailboxAssignTypeSearchObj.MATID = document.frames("TabFrame").document.frames.GetMATID();
}

function IsCurrentSelectionValid()
{
	var MATID = document.frames("TabFrame").document.frames.GetMATID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	MailboxAssignTypeSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "MailboxAssignTypeSearch-f.asp?MODE=<%=MODE%>&SearchAHSID=<%= Request.QueryString("SearchAHSID") %>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "MailboxAssignTypeDetails-f.asp?MODE=<%=MODE%>&MATID=<%=Request.QueryString("MATID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>

<%	If (Request.QueryString("MATID") = "NEW" Or Request.QueryString("MATID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "MailboxAssignTypeDetails-f.asp?MODE=<%=MODE%>&MATID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				UpdateCurrentObjFromScreen();
				MailboxAssignTypeSearchObj.Selected = true;
				window.close();
			}
		}
	}
...
function BtnClose_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		{
			MailboxAssignTypeSearchObj.Selected = false;
			window.close();
		}
	}
}	
...
<object data="..\Scriptlets\TabScriptlet.htm" id="TabsControl" type="text/x-scriptlet"></object>
<% If Request.QueryString("AHSID") <> "" Then %>
<!--#include file="..\lib\NavBack.inc"-->
<% End If %>  
<iframe FRAMEBORDER="0" ID="TabFrame" WIDTH="1000" HEIGHT="700">
</iframe>
