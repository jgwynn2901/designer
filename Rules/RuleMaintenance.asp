...
<%
	...
	If HasModifyPrivilege("FNSD_RULES",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_RULES",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
%>
...
{
	RuleSearchObj.RID = document.frames("TabFrame").document.frames.GetRID();
	RuleSearchObj.RIDText = document.frames("TabFrame").document.frames.GetRIDText();
	RuleSearchObj.RIDType = document.frames("TabFrame").document.frames.GetRIDType();
}

function IsCurrentSelectionValid()
{
	var RID = document.frames("TabFrame").document.frames.GetRID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	RuleSearchObj	= window.dialogArguments;
<%	end if %>

		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "RuleSearch-f.asp?MODE=<%=MODE%>", 1);
<%	End If %>
	AddTab("Details", 120, "RuleEditor-f.asp?MODE=<%=MODE%>&RID=<%=Request.QueryString("RID")%>",1);
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "RuleEditor-f.asp?MODE=<%=MODE%>&RID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
