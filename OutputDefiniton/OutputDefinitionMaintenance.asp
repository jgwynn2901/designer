...
<%
	...
	if CStr(Request.QueryString("CONTAINERTYPE")) <> "" then 
		...
		Session("CONTAINERTYPE") = Request.QueryString("CONTAINERTYPE")
	End If		
	...
	If HasAddPrivilege("FNSD_OUTPUT_DEFINITION",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
var OutputDefinitionSearchObj = new COutputDefinitionSearchObj();
...
{
	var ODID = document.frames("TabFrame").document.frames.GetODID();
	if (ODID == "")
		return false;
	else
		return true;
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	OutputDefinitionSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "OutputDefinitionSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "OutputDefinitionDetails-f.asp?MODE=<%=MODE%>&ODID=<%=Request.QueryString("ODID")%>&VID=<%=Request.QueryString("VID")%>",1);
<%	end if %>
...
{
	OutputDefinitionSearchObj.ODID = document.frames("TabFrame").document.frames.GetODID();
	OutputDefinitionSearchObj.ODIDName = document.frames("TabFrame").document.frames.GetODIDName();
}
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "OutputDefinitionDetails-f.asp?MODE=<%=MODE%>&ODID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSave_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		if (document.frames("TabFrame").document.frames.ExeSave() == true)
		{
			...
			OutputDefinitionSearchObj.Saved = true;
		}
	}
}
	...
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			else
			{
				...
				OutputDefinitionSearchObj.Selected = true;
				window.close();
			}
		}
	}
...
function BtnClose_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		{	
			OutputDefinitionSearchObj.Selected = false;
			window.close();
		}
	}		
}
