...
<%
	...
	If HasModifyPrivilege("FNSD_OUTPUT_OVERFLOW",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_OUTPUT_OVERFLOW",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
function COutputOverflowSearchObj()
{
	this.OOID = "";
	this.Selected = "";
}
var OutputOverflowSearchObj = new COutputOverflowSearchObj();
...
{
	OutputOverflowSearchObj.OOID = document.frames("TabFrame").document.frames.GetOOID();
}

function IsCurrentSelectionValid()
{
	var OOID = document.frames("TabFrame").document.frames.GetOOID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	OutputOverflowSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "OutputOverflowSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "OutputOverflowDetails-f.asp?MODE=<%=MODE%>&OOID=<%=Request.QueryString("OOID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>

<%	If (Request.QueryString("OOID") = "NEW" Or Request.QueryString("OOID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "OutputOverflowDetails-f.asp?MODE=<%=MODE%>&OOID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				UpdateCurrentObjFromScreen();
				OutputOverflowSearchObj.Selected = true;
				window.close();
			}
		}
	}
...
function BtnClose_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		{
			OutputOverflowSearchObj.Selected = false;
			window.close();
		}
	}
}	
