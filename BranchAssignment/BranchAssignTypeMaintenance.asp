...
<%
	...
	If HasModifyPrivilege("FNSD_BRANCH_ASSIGNMENT",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_BRANCH_ASSIGNMENT",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
%>
...
{
	var BATID = document.frames("TabFrame").document.frames.GetBATID();
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "BranchAssignTypeSearch-f.asp?MODE=<%=MODE%>&SearchAHSID=<%= Request.QueryString("SearchAHSID") %>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "BranchAssignTypeDetails-f.asp?MODE=<%=MODE%>&BATID=<%=Request.QueryString("BATID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>

<%	If (Request.QueryString("BATID") = "NEW" Or Request.QueryString("BATID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "BranchAssignTypeDetails-f.asp?MODE=<%=MODE%>&BATID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
