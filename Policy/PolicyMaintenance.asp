...
<%
	...
	If SELECTONLY <> "TRUE" Then
		If HasModifyPrivilege("FNSD_POLICY",SECURITYPRIV) <> True Then MODE = "RO"
	Else
		MODE ="RO"
	End If
	...
	If HasAddPrivilege("FNSD_POLICY",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
<% If Request.QueryString("AHSID") <> "" Then %>
	location.href = "..\AH\NodeSummary.asp?AHSID=<%= Request.QueryString("AHSID")%>&DROPDOWN=POLICY"
<% End If %>
End Sub
</script>
...
{
	PolicySearchObj.PID = document.frames("TabFrame").document.frames.GetPID();
}

function IsCurrentSelectionValid()
{
	var PID = document.frames("TabFrame").document.frames.GetPID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	PolicySearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "PolicySearch-f.asp?<%=Request.QueryString%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "PolicyDetails-f.asp?MODE=<%=MODE%>&PID=<%=Request.QueryString("PID")%>&AHSID=<%=Request.QueryString("AHSID")%>&SearchAHSID=<%= Request.QueryString("SearchAHSID") %>",1);
	//AddTab("Property", 120, "PolicyProperty-f.asp?MODE=<%=MODE%>&PID=<%=Request.QueryString("PID")%>",1);
	AddTab("Vehicle", 120, "PolicyVehicle-f.asp?<%=Request.QueryString("PID")%>", 1);
	AddTab("Location", 120, "LocationDetails-f.asp?MODE=<%=MODE%>&PID=<%=Request.QueryString("PID")%>",1);
<%	end if %>



<%	If (Request.QueryString("PID") = "NEW" Or Request.QueryString("PID") = "") And bShowSearchTab <> false Then %>
	...
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		if (DoOnscriptletevent(theEvent,theData) == true)
		{
			switch (theData)
			{
				...
				case "3"	:	//vehicle
					{
					ConfigureButtonsOnActivateTab("Vehicle");
					break;
					}
				case "4"	:	//location
					ConfigureButtonsOnActivateTab("Location");
			}
		}
	}
	else
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "PolicyDetails-f.asp?MODE=<%=MODE%>&PID=NEW&SearchAHSID=<%= SearchAHSID%>");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
