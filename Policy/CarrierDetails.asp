...
<%
	Dim SharedCount, SharedCountText, CID
	SharedCount = 0
	...
	If CID <> "" Then
		If CID = "NEW" Then 
			SharedCount = 0
		Else
			SharedCount = CheckSharedCarrier(CLng(CID),True,True,1,False,False,0)
		End If
	End If	
...
If CID <> "" Then
	If CID <> "NEW" then
		...
		SQLST = "SELECT * FROM CARRIER WHERE CARRIER_ID = " & CID
		...
		If Not RS.EOF then
			...
			RSPHONE = RS("PHONE")
			RSFEIN	= RS("FEIN")
			RSCARRIERNUMBER = RS("CARRIER_NUMBER")
...
Sub PostTo(strURL)
	FrmDetails.action = "CarrierSearch-f.asp"
...
Function GetCIDName
	GetCIDName = document.all.TxtName.value
End Function
...
Function ExeCopy
	...
	if document.all.CID.value = "" then
		ExeCopy = false
		exit function
	end if

	document.all.CID.value = "NEW"
	document.body.setAttribute "ScreenDirty", "YES"
...
Function ExeSave
	...
	'If CStr(document.body.getAttribute("ScreenDirty")) = "YES" Then
		...
		sResult = sResult & "CARRIER_ID"& Chr(129) & document.all.CID.value & Chr(129) & "0" & Chr(128)
		...
		sResult = sResult & "CARRIER_NUMBER"& Chr(129) & document.all.TxtCarrierNumber.value & Chr(129) & "1" & Chr(128)
...
Sub RefCountRpt_onclick()
	If document.all.SpanSharedCount.innerText > 0 Then
		If document.all.CID.value <> "" And document.all.CID.value <> "NEW" Then
			paramID = document.all.CID.value
		Else	
			paramID = 0
		End If
		lret = window.showModalDialog ("..\StatusRpt\RefCountRpt.asp?CheckSharedCarrier=True&ID=" & paramID, Null,  "dialogWidth=580px; dialogHeight=400px; center=yes")
	Else
...
<tr><td CLASS="GrpLabel" WIDTH="134" HEIGHT="10"><nobr>&nbsp;» Carrier Details&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img SRC="../Images/HelpIcon2.gif" STYLE="CURSOR:HAND" align="absmiddle" title="Help" OnClick="LaunchHelp(&quot;Welcome.htm&quot;)" WIDTH="7" HEIGHT="8"></td>
...
<form Name="FrmDetails" METHOD="POST" ACTION="CarrierSave.asp" TARGET="hiddenPage">
<input TYPE="HIDDEN" NAME="TxtSaveData">
<input TYPE="HIDDEN" NAME="TxtAction">

<% 'need to maintain these values in order to post back to the search tab %>
<input type="hidden" name="SearchCID" value="<%=Request.QueryString("SearchCID")%>">
...
<input type="hidden" name="SearchCarrierNumber" value="<%=Request.QueryString("SearchCarrierNumber")%>">
...
<table class="LABEL">
	<tr>
	<td CLASS="LABEL">Carrier ID:&nbsp;<span id="spanCID"><%=Request.QueryString("CID")%></span></td>
	</tr>
	<tr>
	...
	<tr>
		<table>
		<tr>
			<td CLASS="LABEL">Carrier Number:<br><input ScrnInput="TRUE" CLASS="LABEL" MAXLENGTH="30" size="30" TYPE="TEXT" NAME="TxtCarrierNumber" VALUE="<%=RSCARRIERNUMBER%>" ONKEYPRESS="VBScript::Control_OnChange" ONCHANGE="VBScript::Control_OnChange"></td>
