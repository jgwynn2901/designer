...
<%
	...
	If HasModifyPrivilege("FNSD_AGENT",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_AGENT",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
{
	AgentSearchObj.AID = document.frames("TabFrame").document.frames.GetAID();
	AgentSearchObj.AIDName = document.frames("TabFrame").document.frames.GetAIDName();
}
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	AgentSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "AgentSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "AgentDetails-f.asp?MODE=<%=MODE%>&AID=<%=Request.QueryString("AID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "AgentDetails-f.asp?MODE=<%=MODE%>&AID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
