...
<%
	...
	If HasModifyPrivilege("FNSD_CARRIER",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_CARRIER",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
%>
...
{
	CarrierSearchObj.CID = document.frames("TabFrame").document.frames.GetCID();
	CarrierSearchObj.CIDName = document.frames("TabFrame").document.frames.GetCIDName();
}

function IsCurrentSelectionValid()
{
	var CID = document.frames("TabFrame").document.frames.GetCID();
	if (CID == "")
		return false;
	else
		return true;
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "CarrierSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "CarrierDetails-f.asp?MODE=<%=MODE%>&CID=<%=Request.QueryString("CID")%>",1);
<%	end if %>

<%	If (Request.QueryString("CID") = "NEW" Or Request.QueryString("CID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "CarrierDetails-f.asp?MODE=<%=MODE%>&CID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
