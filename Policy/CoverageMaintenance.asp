...
<%
	...
	If HasModifyPrivilege("FNSD_COVERAGE",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_COVERAGE",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
{
	CoverageSearchObj.COVID = document.frames("TabFrame").document.frames.GetCOVID();
}

function IsCurrentSelectionValid()
{
	var COVID = document.frames("TabFrame").document.frames.GetCOVID();
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "CoverageSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "CoverageDetails-f.asp?MODE=<%=MODE%>&<%=Request.QueryString %>",1);
<%	end if %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "CoverageDetails-f.asp?MODE=<%=MODE%>&COVID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
