...
<%
	...
	If HasModifyPrivilege("FNSD_PROPERTY",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_PROPERTY",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
{
	var PROPID = document.frames("TabFrame").document.frames.GetPROPID();
	if (PROPID == "")
		return false;
	else
		return true;
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	PropertySearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "PropertySearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "PropertyDetails-f.asp?MODE=<%=MODE%>&PROPID=<%=Request.QueryString("PROPID")%>&PID=<%=Request.QueryString("PID")%>",1);
<%	end if %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "PropertyDetails-f.asp?MODE=<%=MODE%>&PROPID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
	...
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			else
			{
				...
				PropertySearchObj.Selected = true;
				window.close();
			}
		}
	}
...
function BtnClose_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		{	
			PropertySearchObj.Selected = false;
			window.close();
		}
	}		
}
