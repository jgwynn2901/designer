...
<%
	...
	If HasModifyPrivilege("FNSD_TPA",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_TPA",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
%>
...
{
	TPASearchObj.TPAID = document.frames("TabFrame").document.frames.GetTPAID();
	TPASearchObj.TPAIDName = document.frames("TabFrame").document.frames.GetTPAIDName();
}

function IsCurrentSelectionValid()
{
	var TPAID = document.frames("TabFrame").document.frames.GetTPAID();
	if (TPAID == "")
		return false;
	else
		return true;
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	TPASearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "TPASearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "TPADetails-f.asp?MODE=<%=MODE%>&TPAID=<%=Request.QueryString("TPAID")%>",1);
<%	end if %>

<%	If (Request.QueryString("TPAID") = "NEW" Or Request.QueryString("TPAID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "TPADetails-f.asp?MODE=<%=MODE%>&TPAID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
