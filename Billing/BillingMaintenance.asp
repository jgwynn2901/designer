...
<%
	...
	If HasModifyPrivilege("FNSD_FEE",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_FEE",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "BillingSearch-f.asp?MODE=<%=MODE%>&AHSID=<%= Request.QueryString("AHSID") %>&SearchAHSID=<%= Request.QueryString("SearchAHSID") %>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "BillingDetails-f.asp?MODE=<%=MODE%>&BID=<%=Request.QueryString("BID")%>&AHSID=<%= Request.QueryString("AHSID") %>",1);
<%	end if %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "BillingDetails-f.asp?MODE=<%=MODE%>&BID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				window.dialogArguments.BillingID = document.frames("TabFrame").document.frames.GetBID();
				window.close();
			}
		}
	}
...
<% If Request.QueryString("CONTAINERTYPE") <> "MODAL" AND Request.QueryString("AHSID") <> "" Then %>
