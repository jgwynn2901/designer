...
<%	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "VendorReferalDetails-f.asp?MODE=<%=MODE%>&BATID=<%=Request.QueryString("BATID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "VendorReferalDetails-f.asp?MODE=<%=MODE%>&BATID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
