...
function CClaimKOCSearchObj()
{
	this.KOCID = "";
	this.Selected = "";
}
var ClaimKOCSearchObj = new CClaimKOCSearchObj();
...
{
	ClaimKOCSearchObj.KOCID = document.frames("TabFrame").document.frames.GetKOCID();
}

function IsCurrentSelectionValid()
{
	var KOCID = document.frames("TabFrame").document.frames.GetKOCID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	ClaimKOCSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "ClaimKOCSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "ClaimKOCDetails-f.asp?MODE=<%=MODE%>&KOCID=<%=Request.QueryString("KOCID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>

<%	If (Request.QueryString("KOCID") = "NEW" Or Request.QueryString("KOCID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "ClaimKOCDetails-f.asp?MODE=<%=MODE%>&KOCID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				UpdateCurrentObjFromScreen();
				ClaimKOCSearchObj.Selected = true;
				window.close();
			}
		}
	}
...
function BtnClose_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		{
			ClaimKOCSearchObj.Selected = false;
			window.close();
		}
	}
}	
