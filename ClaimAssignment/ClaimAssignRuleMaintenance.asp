...
<%
	...
	If HasModifyPrivilege("FNSD_CLAIM_ASSIGNMENT",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_CLAIM_ASSIGNMENT",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
%>
...
function CClaimAssignRuleSearchObj()
{
	this.CARID = "";
	this.Selected = "";
}
var ClaimAssignRuleSearchObj = new CClaimAssignRuleSearchObj();
...
{
	ClaimAssignRuleSearchObj.CARID = document.frames("TabFrame").document.frames.GetCARID();
}
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	ClaimAssignRuleSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "ClaimAssignRuleSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "ClaimAssignRuleDetails-f.asp?MODE=<%=MODE%>&CARID=<%=Request.QueryString("CARID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "ClaimAssignRuleDetails-f.asp?MODE=<%=MODE%>&CARID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				UpdateCurrentObjFromScreen();
				ClaimAssignRuleSearchObj.Selected = true;
				window.close();
			}
		}
	}
...
function BtnClose_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		{
			ClaimAssignRuleSearchObj.Selected = false;
			window.close();
		}
	}
}	
