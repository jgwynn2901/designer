...
<%	
	...
	IF Request.QueryString("COID") <> "NEW" Then
		...
		S_Query = "Select * From AHS_CONTACT WHERE CONTACT_ID =" &  Request.QueryString("COID") & " order by ahs_contact_id asc"
	...
	IF Request.QueryString("Action") = "Delete" and IsNumeric(Request.QueryString("Delete_ID")) Then
		...
		response.redirect("SEDContactDetailsaHSSummary.asp?MODE=" & Request.QueryString("MODE") & "&COID=" & Request.QueryString("COID")& "&ClientCode=" & Request.QueryString("ClientCode"))
...
Function EditClick()
	...
	if 0 < i then
		s_URL = "AHSContact-f.asp" & "?MODE=" & "EDIT" & "&AHSCID=" & GetHightLightedAhsContact_ID(Document.all.tbl_SeqStep.rows(i)) & "&COID=" &  "<%=Request.QueryString("COID")%>" & "&ClientCode=" & "<%=Request.QueryString("ClientCode")%>"
		l_Ret = window.showModalDialog(s_URL,Null,"dialogWidth:450px;dialogHeight:300px;center")
		
		self.location.href = "SEDContactDetailsAHSSummary.asp?Mode=" & "<%=Mode%>" & "&COID=" & "<%=Request.QueryString("COID")%>" & "&ClientCode=" & "<%=Request.QueryString("ClientCode")%>"
	end if
End Function
...
Function NewClick()
	...
	If "<%=Mode%>" = "RO" Then
		...
		Exit Function
	End If
	s_URL = "AHSContact-f.asp" & "?MODE=" & "NEW" & "&AHSCID=NEW" & "&COID=" &  "<%=Request.QueryString("COID")%>" & "&ClientCode=" & "<%=Request.QueryString("ClientCode")%>"
	l_Ret = window.showModalDialog(s_URL,Null,"dialogWidth:450px;dialogHeight:300px;center")
...
Function DeleteClick()
	...
	i = getselectedindex( Document.all.tbl_SeqStep )
	if 0 < i then
	     s_url = "SEDContactDetailsAHSSummary.asp?Mode=" & "<%=Mode%>" & "&Action=Delete&Delete_ID=" & GetHightLightedAhsContact_ID(Document.all.tbl_SeqStep.rows(i)) &  "&COID=" & "<%=Request.QueryString("COID")%>" & "&ClientCode=" & "<%=Request.QueryString("ClientCode")%>" 
  ...
  <TABLE cellPadding=2 cellSpacing=0 rules=all ID="tbl_SeqStep" name="tbl_SeqStep"  Width="100%">
	 <THead CLASS="ResultHeader">
		<tr align="left" >
			...
			<td class=thd><div id="LOB"><NOBR>LOB</div></td>
		
		</tr>
	</THead>
	<TBody ID="TableRows">
<% Do While Not rs.EOF  %>
		<tr ID="rowData" CLASS="ResultRow" AHSCID="<%=rs("AHS_CONTACT_ID")%>" COID="<%=rs("CONTACT_ID")%>" AHSID="<%=rs("ACCNT_HRCY_STEP_ID")%>" AEDDT="<%=rs("ACTIVE_END_DT")%>" ASTDT="<%=rs("ACTIVE_START_DT")%>" ClientCode="<%=ClientCode%>" OnClick="Javascript:multiselect(this);" OnDblClick="Javascript:dblclick(this);">
