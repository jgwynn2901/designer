...
<%
	...
	Dim s_SeqStepID,s_DestinationID,s_SeqNumber ,s_RetryCount ,s_RetryWaitTime ,s_DestinationStr,s_Transmission, s_Lob 
...
<tr></tr>
	<tr><td>	
		<Table CLASS="LABEL" cellpadding="2" cellspacing="2">
          ...
          <td class = label >LOB:<br> <select NAME="txtlob"   CLASS="LABEL" ID="Select2" ONCHANGE="VBScript::Control_OnChange"><%=GetControlDataHTML("LOB","LOB_CD","LOB_CD",S_LOB,true)%>
        <!--  <option value =""></option>
...
ACTION="SEDContactDetailsAHSSummaryModal.asp?CHECK=Save&MODE=EDIT&AHSCID=<%=AHSCID%>&COID=<%=Request.Querystring("COID")%>" 
<% else %>
ACTION="SEDContactDetailsAHSSummaryModal.asp?CHECK=Save&MODE=NEW&COID=<%=Request.Querystring("COID")%>" 
<% end if%>
METHOD="POST" >
