...
<%
	...
	If HasModifyPrivilege("FNSD_OWNER",SECURITYPRIV) <> True Then MODE = "RO"
	...
	bShowNew = true
	'***************
	'DMS: 4/6 : The make copy button needs to be disabled for owner screen
	'***************
	bShowCopy = false
	bShowSave = true 
	bShowSelect = true
	...
	If HasAddPrivilege("FNSD_OWNER",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
{
	CarrierSearchObj.OID = document.frames("TabFrame").document.frames.GetOID();
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "OwnerSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "OwnerDetails-f.asp?MODE=<%=MODE%>&OID=<%=Request.QueryString("SearchOID")%>",1);
<%	end if %>

<%	If (Request.QueryString("SearchOID") = "NEW" Or Request.QueryString("SearchOID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "OwnerDetails-f.asp?MODE=<%=MODE%>&SearchOID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
