...
<%
	...
	If HasModifyPrivilege("FNSD_ESCALATION",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_ESCALATION",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
{
	CarrierSearchObj.EPID = document.frames("TabFrame").document.frames.GetEPID();
	CarrierSearchObj.EPIDName = document.frames("TabFrame").document.frames.GetEPIDName();
}

function IsCurrentSelectionValid()
{
	var EPID = document.frames("TabFrame").document.frames.GetEPID();
	if (EPID == "")
		return false;
	else
		return true;
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "EscalationSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "EscalationDetails-f.asp?MODE=<%=MODE%>&EPID=<%=Request.QueryString("EPID")%>",1);
<%	end if %>

<%	If (Request.QueryString("EPID") = "NEW" Or Request.QueryString("EPID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "EscalationDetails-f.asp?MODE=<%=MODE%>&EPID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
