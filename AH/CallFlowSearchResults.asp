...
var qry
	...
	alert (objRow.getAttribute("AHSID"))
}
-->
</script>
...
If Request.Form <> "" Then
	...
	SQLST = ""
	SQLWhere = ""
	SQLST = SQLST & "SELECT CALLFLOW.NAME, CALLFLOW.DESCRIPTION, ACCOUNT_CALLFLOW.LOB_CD, ACCOUNT_CALLFLOW.ACCNT_HRCY_STEP_ID FROM CALLFLOW,ACCOUNT_CALLFLOW WHERE "
	SQLWhere = SQLWhere & " CALLFLOW.CALLFLOW_ID = ACCOUNT_CALLFLOW.CALLFLOW_ID "
	'For Each x In Request.Form 
	'	If Request.Form(x) <> "" Then
	'			If SQLWhere <> "" Then
	'				SQLWhere = SQLWhere & " AND "
	'			End If
	'		SQLWhere = SQLWhere & x & "='" & Request.Form(x) & "'"

	'	End If
	'Next	
	If Request.form("NAME") <> "" Then
		SQLWhere = SQLWhere & " AND UPPER(CALLFLOW.NAME) LIKE '" & UCASE(Request.Form("NAME")) & "%'"
	End If
	If Request.form("LOB_CD") <> "" Then
		SQLWhere = SQLWhere & " AND ACCOUNT_CALLFLOW.LOB_CD='" & UCASE(Request.Form("LOB_CD")) & "'"
	End If
	If Request.form("DESCRIPTION") <> "" Then
		SQLWhere = SQLWhere & "AND UPPER(CALLFLOW.DESCRIPTION) LIKE '" & UCASE(Request.Form("DESCRIPTION")) & "%'"
	End If
		Response.Write(SQLST & SQLWhere)
	Set RS = Conn.Execute(SQLST & SQLWhere)
	Do While Not RS.EOF
%>
<TR CLASS=RESULTROW OnClick="Javascript:multiselect(this);" OnDblClick="Javascript:dblhighlight(this);" AHSID='<%= RS("ACCNT_HRCY_STEP_ID") %>'>
<TD CLASS=LABEL><%= RenderCell(RS("NAME")) %></TD>
<TD CLASS=LABEL><%= RenderCell(RS("DESCRIPTION")) %></TD>
<TD CLASS=LABEL><%= RenderCell(RS("LOB_CD")) %></TD>
</TR>
<%
RS.movenext
Loop
...
%>
<TR>
<TD CLASS=RESULTROW COLSPAN=5>No Call Flows Selected</TD>
</TR>
<%
End If
RS.Close
