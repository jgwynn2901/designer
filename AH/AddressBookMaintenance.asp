...
<%
	...
	If HasModifyPrivilege("FNSD_ADDRESS_BOOK",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_ADDRESS_BOOK",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
		bShowDelete = false
	End If
...
{
	var ABID = document.frames("TabFrame").document.frames.GetABID();
	if (ABID == "")
		return false;
	else
		return true;
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "AddressBookSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "AddressBookDetails-f.asp?MODE=<%=MODE%>&ABID=<%=Request.QueryString("ABID")%>&VID=<%=Request.QueryString("VID")%>",1);
<%	end if %>
...
{
	DriverSearchObj.ABID = document.frames("TabFrame").document.frames.GetABID();
}
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "AddressBookDetails-f.asp?MODE=<%=MODE%>&ABID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
