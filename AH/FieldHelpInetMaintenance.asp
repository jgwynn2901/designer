...
<%
	...
	If HasModifyPrivilege("FNSD_FIELD_HELP_INETINTERNAL",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_FIELD_HELP_INETINTERNAL",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
	If HasDeletePrivilege("FNSD_FIELD_HELP_INETINTERNAL",SECURITYPRIV) <> True Then
		bShowDelete = false
	End If		
...
{
	CarrierSearchObj.HPID = document.frames("TabFrame").document.frames.GetHPID();
	CarrierSearchObj.HPIDName = document.frames("TabFrame").document.frames.GetEPIDName();
}

function IsCurrentSelectionValid()
{
	var HPID = document.frames("TabFrame").document.frames.GetHPID();
	if (HPID == "")
		return false;
	else
		return true;
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "FieldHelpInetSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "FieldHelpInetDetails-f.asp?MODE=<%=MODE%>&HPID=<%=Request.QueryString("HPID")%>",1);
<%	end if %>

<%	If (Request.QueryString("HPID") = "NEW" Or Request.QueryString("HPID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "FieldHelpInetDetails-f.asp?MODE=<%=MODE%>&HPID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
<OBJECT data=..\Scriptlets\Tabscriptlet.asp id=TabsControl style="LEFT: 0px; TOP: 0px" 
