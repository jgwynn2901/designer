...
<%
	...
	If HasModifyPrivilege("FNSD_EMPLOYEE",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_EMPLOYEE",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
function CEmployeeSearchObj()
{
	this.EID = "";
	this.Selected = false;	
	this.Saved = false;	
}
var EmployeeSearchObj = new CEmployeeSearchObj();
...
{
	EmployeeSearchObj.EID = document.frames("TabFrame").document.frames.GetEID();
}
...
{
	var EID = document.frames("TabFrame").document.frames.GetEID();
	if (EID == "")
		return false;
	else
		return true;
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	EmployeeSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "EmployeeSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "EmployeeDetails-f.asp?MODE=<%=MODE%>&EID=<%=Request.QueryString("EID")%>&PID=<%=Request.QueryString("PID")%>",1);
<%	end if %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "EmployeeDetails-f.asp?MODE=<%=MODE%>&EID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				EmployeeSearchObj.Selected = true;
				UpdateCurrentObjFromScreen();
				window.dialogArguments.EmployeeID = document.frames("TabFrame").document.frames.GetEID();
				window.close();
			}
		}
	}
...
function BtnClose_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		{	
			EmployeeSearchObj.Selected = false;
			window.dialogArguments.EmployeeID=""
			window.close();
		}
	}		
}
