...
If Request.QueryString("DELETED") <> "" Then
	...
	SQLDEL=  "DELETE FROM SUBROGATION_DETECTION_RULE WHERE SUBROGATION_DETECTION_TYPE_ID = " & Request.QueryString("DELETED")
	Set RS = Conn.Execute(SQLDEL)
	SQLDEL1=  "DELETE FROM SUBROGATION_DETECTION_TYPE WHERE SUBROGATION_DETECTION_TYPE_ID = " & Request.QueryString("DELETED")
	...
	If strError = "" Then 
		Response.Redirect "SubrogationSummary.asp?AHSID=" & Request.QueryString("AHSID")
	End If
...
cSQL = "SELECT * FROM SUBROGATION_DETECTION_TYPE FDT WHERE " &_
...
function dblhighlight( objRow )
{
	...
	return objRow.getAttribute("STID");
}
</SCRIPT>
...
Function EditClick()
	...
	if 0 < i then
		parent.frames.window.location = "../Subrogation/SubroMaintenance.asp?CONTEXTTYPE=DRILLIN&CONTAINERTYPE=FRAMEWORK&DETAILONLY=TRUE&STID=" & dblhighlight(Document.all.tblResult.rows(i)) & "&AHSID=<%= Request.QueryString("AHSID") %>"
	end if
End Function

Function NewClick()
    parent.frames.window.location = "../Subrogation/SubroMaintenance.asp?CONTEXTTYPE=DRILLIN&CONTAINERTYPE=FRAMEWORK&STID=NEW&DETAILONLY=TRUE&AHSID=<%= Request.QueryString("AHSID") %>"
End Function
...
Function RemoveClick()
	...
	if 0 < i then
		...
		STID = dblhighlight(Document.all.tblResult.rows(i))
		lret = MsgBox ("Are you sure you want to delete Subrogation Type ID:" & STID & " for Account ID:<%=Request.QueryString("AHSID")%>?", 1, "FNSDesigner")
		If lret = 1 Then
	       self.location.href = "SubrogationSummary.asp?DELETED=" & STID & "&AHSID=<%= Request.QueryString("AHSID") %>"
		End If
...
<table cellPadding=2 cellSpacing=0  rules=all ID="tblResult" name="tblResult" width=100%>
	<thead CLASS="ResultHeader">
		<tr align="left">
			<td class=thd><div><NOBR>Subro ID</div></td>
...
<% Do While Not oRS.EOF %>
	<tr ID="FieldRow" CLASS="ResultRow" OnClick="Javascript:multiselect(this);" OnDblClick="Javascript:dblclick(this);" STID='<%= oRS("SUBROGATION_DETECTION_TYPE_ID") %>'>
			<td NOWRAP CLASS=ResultCell><%= renderCell(oRS("SUBROGATION_DETECTION_TYPE_ID")) %></td>
