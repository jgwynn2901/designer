...
<%
	...
	If HasModifyPrivilege("FNSD_ACC_VENDOR",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_ACC_VENDOR",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
<%	end if 
	...
	AddTab("Details", 120, "AccVendorDetails-f.asp?<%=Request.QueryString%>",1);
<%	end if %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "AccVendorDetails-f.asp?MODE=<%=MODE%>&BID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
