...
<%
	...
	If HasModifyPrivilege("FNSD_CONTACT",SECURITYPRIV) <> True Then MODE = "RO"
	...
	bShowNew = true
	'***************
	'DMS: 4/6 : The make copy button needs to be disabled for contact screen
	'***************
	bShowCopy = false
	bShowSave = true 
	bShowSelect = true
	...
	If HasAddPrivilege("FNSD_CONTACT",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
{
	var COID = document.frames("TabFrame").document.frames.GetCOID();
	if (COID == "")
		return false;
	else
		return true;
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "ContactSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "ContactDetails-f.asp?MODE=<%=MODE%>&COID=<%=Request.QueryString("COID")%>",1);
<%	end if %>

<%	If (Request.QueryString("COID") = "NEW" Or Request.QueryString("COID") = "") And bShowSearchTab <> false Then %>
