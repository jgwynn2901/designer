...
<%	Response.Expires=0
	...
	Dim STID, SRID, isRequired
	
	STID =  CStr(Request.QueryString("STID"))
	SRID =  CStr(Request.QueryString("SRID"))
...
<title>Subrogation Rule Details</title>
...
Sub UpdateSRID(inSRID)
	document.all.SRID.value = inSRID
	document.all.spanSRID.innerText = inSRID
	if document.all.spanSRID.innerText <> "NEW" then
...
Function GetSRID
	if document.all.SRID.value <> "NEW" then
	    GetSRID = document.all.SRID.value
	else
		GetSRID = ""
	end if 
End Function
...
'	If CStr(document.body.getAttribute("ScreenDirty")) = "YES" Then
		...
		sResult = sResult & "SUBROGATION_DETECTION_RULE_ID " & Chr(129) & document.all.SRID.value & Chr(129) & "1" & Chr(128)
		sResult = sResult & "SUBROGATION_DETECTION_TYPE_ID " & Chr(129) & document.all.STID.value & Chr(129) & "1" & Chr(128)
...
<tr><td CLASS="GrpLabel" WIDTH="134" HEIGHT="10"><nobr>&nbsp;» Subrogationn Details&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img SRC="../Images/HelpIcon2.gif" STYLE="CURSOR:HAND" align="absmiddle" title="Help" OnClick="LaunchHelp(&quot;Welcome.htm&quot;)" WIDTH="7" HEIGHT="8"></td>
...
<form Name="FrmDetails" METHOD="POST" ACTION="SubroRuleSave.asp" TARGET="hiddenPage">
...
If SRID <> "" Then
	If SRID <> "NEW" Then
		...
		SQLST = "SELECT SR.*, R.RULE_TEXT " 
		SQLST=SQLST & "FROM SUBROGATION_DETECTION_RULE SR, RULES R "
		SQLST=SQLST & "WHERE SR.RULE_ID = R.RULE_ID(+) " 
		SQLST=SQLST & "AND SR.SUBROGATION_DETECTION_RULE_ID = " & SRID 
...
<tr></tr>
<tr>
	<td COLSPAN="3">Subrogation Rule ID:&nbsp;<span id="spanSRID"><%=Request.QueryString("SRID")%></span></td></tr>
<tr>
