...
<% 	Response.Expires = 0
	...
	If HasModifyPrivilege("FNSD_SUBROGATION_DETECTION",SECURITYPRIV) <> True Then MODE = "RO"
...
{
	var STID = document.frames("TabFrame").document.frames.GetSTID();
...
<%	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "SubroDetails-f.asp?MODE=<%=MODE%>&STID=<%=Request.QueryString("STID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>

<%	If (Request.QueryString("STID") = "NEW" Or Request.QueryString("STID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "SubroDetails-f.asp?MODE=<%=MODE%>&STID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
