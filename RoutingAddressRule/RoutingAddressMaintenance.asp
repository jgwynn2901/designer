...
<title>Routing Address Maintenance</title>
<%	if CONTAINERTYPE = "MODAL" then %>
<STYLE TYPE="text/css">
HTML {width: 450pt; height: 350pt}
</STYLE> 
<%	end if %>
...
{
	RoutingAddressSearchObj.RAID = document.frames("TabFrame").document.frames.GetRAID();
	RoutingAddressSearchObj.RAIDDescription = document.frames("TabFrame").document.frames.GetRAIDDescription();
	RoutingAddressSearchObj.RAIDState = document.frames("TabFrame").document.frames.GetRAIDState();
	RoutingAddressSearchObj.RAIDFIPS = document.frames("TabFrame").document.frames.GetRAIDFIPS();
	RoutingAddressSearchObj.RAIDZip = document.frames("TabFrame").document.frames.GetRAIDZip();		
}

function IsCurrentSelectionValid()
{
	var RAID = document.frames("TabFrame").document.frames.GetRAID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	RoutingAddressSearchObj	= window.dialogArguments;
<%	end if %>

		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "RoutingAddressSearch-f.asp?MODE=<%=MODE%>", 1);
<%	End If %>
	AddTab("Details", 120, "RoutingAddressDetails-f.asp?MODE=<%=MODE%>&RAID=<%=Request.QueryString("RAID")%>",1);

<%	If (Request.QueryString("RAID") = "NEW" Or Request.QueryString("RAID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "RoutingAddressDetails-f.asp?MODE=<%=MODE%>&RAID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				UpdateCurrentObjFromScreen();
				RoutingAddressSearchObj.Selected = true;
				window.close();
			}
		}
	}		
