...
<%
	...
	If HasModifyPrivilege("FNSD_FRAUD_DETECTION",SECURITYPRIV) <> True Then MODE = "RO"
...
{
	var FDTID = document.frames("TabFrame").document.frames.GetFDTID();
...
<%	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "FraudDetDetails-f.asp?MODE=<%=MODE%>&FDTID=<%=Request.QueryString("FDTID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>

<%	If (Request.QueryString("FDTID") = "NEW" Or Request.QueryString("FDTID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "FraudDetDetails-f.asp?MODE=<%=MODE%>&FDTID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
