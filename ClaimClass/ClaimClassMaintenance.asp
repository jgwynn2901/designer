...
'***************************************************************
'General purpose: Maintenance functions for allowing add/save etc. for claim class assignment
'
'$History: ClaimClassMaintenance.asp $ 
'* 
'* *****************  Version 1  *****************
'* User: Alex.shimberg Date: 4/10/06    Time: 11:01p
...
'* New Claim Class Assignment module: Search, Details etc.
	...
	If HasModifyPrivilege("FNSD_CLAIMCLASS_ASSIGNMENT",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_CLAIMCLASS_ASSIGNMENT",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
%>
...
<% If Request.QueryString("AHSID") <> "" Then %>
	location.href = "..\AH\NodeSummary.asp?AHSID=<%= Request.QueryString("AHSID")%>&DROPDOWN=CLAIM"
<% End If %>
End Sub
</script>
...
function CClaimClassAssignRuleSearchObj()
{
	this.CARID = "";
	this.Selected = "";
}
var ClaimClassAssignRuleSearchObj = new CClaimClassAssignRuleSearchObj();
...
{
	ClaimClassAssignRuleSearchObj.CARID = document.frames("TabFrame").document.frames.GetCARID();
}

function IsCurrentSelectionValid()
{
	var CARID = document.frames("TabFrame").document.frames.GetCARID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	ClaimClassAssignRuleSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "ClaimClassAssignRuleSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "ClaimClassAssignRuleDetails-f.asp?MODE=<%=MODE%>&CARID=<%=Request.QueryString("CARID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>

<%	If (Request.QueryString("CARID") = "NEW" Or Request.QueryString("CARID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "ClaimClassAssignRuleDetails-f.asp?MODE=<%=MODE%>&CARID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				UpdateCurrentObjFromScreen();
				ClaimClassAssignRuleSearchObj.Selected = true;
				window.close();
			}
		}
	}
...
function BtnClose_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		{
			ClaimClassAssignRuleSearchObj.Selected = false;
			window.close();
		}
	}
}	
