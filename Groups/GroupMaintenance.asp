...
<%
	...
	If HasAutomaticSecurityPrivilege() = False Then
		If HasModifyPrivilege("FNSD_GROUPS",SECURITYPRIV) <> True Then MODE = "RO"
	End If		
	...
	If HasAutomaticSecurityPrivilege() = False Then
		If HasAddPrivilege("FNSD_GROUPS",SECURITYPRIV) <> True Then
			bShowNew = false
			bShowCopy = false
		End If
	End If
...
function CGroupSearchObj()
{
	this.GID = "";
	this.GIDName = "";
	this.Selected = "";
}
var GroupSearchObj = new CGroupSearchObj();
...
{
	GroupSearchObj.GID = document.frames("TabFrame").document.frames.GetGID();
	GroupSearchObj.GIDName = document.frames("TabFrame").document.frames.GetGIDName();
}

function IsCurrentSelectionValid()
{
	var GID = document.frames("TabFrame").document.frames.GetGID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	GroupSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "GroupSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "GroupDetails-f.asp?MODE=<%=MODE%>&GID=<%=Request.QueryString("GID")%>",1);
<%	end if %>


	AddTab("Permissions", 120, "GroupPermissions-f.asp?MODE=<%=MODE%>&GID=<%=Request.QueryString("GID")%>",1);

<%	If (Request.QueryString("GID") = "NEW" Or Request.QueryString("GID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "GroupDetails-f.asp?MODE=<%=MODE%>&GID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
