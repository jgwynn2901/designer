...
<%
	...
	If HasModifyPrivilege("FNSD_MC_BRANCH_ASSIGNMENT",SECURITYPRIV) <> True Then MODE = "RO"
	...
	If HasAddPrivilege("FNSD_MC_BRANCH_ASSIGNMENT",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
%>
...
{
	MCBranchAssignTypeSearchObj.MCBATID = document.frames("TabFrame").document.frames.GetMCBATID();
}

function IsCurrentSelectionValid()
{
	var MCBATID = document.frames("TabFrame").document.frames.GetMCBATID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	MCBranchAssignTypeSearchObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "MCBranchAssignTypeSearch-f.asp?MODE=<%=MODE%>&SearchAHSID=<%= Request.QueryString("SearchAHSID") %>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "MCBranchAssignTypeDetails-f.asp?MODE=<%=MODE%>&MCBATID=<%=Request.QueryString("MCBATID")%>&AHSID=<%=Request.QueryString("AHSID")%>",1);
<%	end if %>

<%	If (Request.QueryString("MCBATID") = "NEW" Or Request.QueryString("MCBATID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "MCBranchAssignTypeDetails-f.asp?MODE=<%=MODE%>&MCBATID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				UpdateCurrentObjFromScreen();
				MCBranchAssignTypeSearchObj.Selected = true;
				window.close();
			}
		}
	}
...
function BtnClose_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		{
			MCBranchAssignTypeSearchObj.Selected = false;
			window.close();
		}
	}
}	
