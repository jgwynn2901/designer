...
<title>Managed Care Branch Assignment Type Details</title>
...
Function AttachAccount (ID, SPANID)
	...
	strURL = "..\AH\AHSMaintenance.asp?CONTAINERTYPE=MODAL&SECURITYPRIV=FNSD_MC_BRANCH_ASSIGNMENT&SELECTONLY=TRUE&AHSID=" &AHSID
	if MODE = "RO" then strURL = strURL & "&DETAILONLY=TRUE"
...
Sub PostTo(strURL)
	FrmDetails.action = "MCBranchAssignTypeSearch-f.asp"
...
Sub UpdateMCBATID(inMCBATID)
	document.all.MCBATID.value = inMCBATID
	document.all.spanMCBATID.innerText = inMCBATID
'''	If IsThisNew = "Y" Then
		strURL = "MCBranchAssignRuleModal.asp?MCBATID=" & inMCBATID & "&MCBARID=NEW" & "&MODE=" & MODE & "&RequiredMsg=Y"
		showModalDialog  strURL,MCBranchAssignRuleSearchObj ,"center"
'''	End if
	If MCBranchAssignRuleSearchObj.Selected = true Then	Refresh
End Sub
...
Function GetMCBATID
	if document.all.MCBATID.value <> "NEW" then
		GetMCBATID = document.all.MCBATID.value
	else
		GetMCBATID = ""
	end if 
End Function
...
Function GetSelectedMCBARID
	GetSelectedMCBARID = document.frames("DataFrame").GetSelectedMCBARID
End Function
...
Sub ExeNewBranchRule
	...
	If document.all.MCBATID.value = "" Or document.all.MCBATID.value = "NEW" Then
		Exit Sub
	End If

<%If HasAddPrivilege("FNSD_MC_BRANCH_ASSIGNMENT","") <> True Then  %>		
		MsgBox "You do not have the appropriate security privileges to add managed care branch assignment rules.",0,"FNSNetDesigner"
		Exit Sub
<%End If %>		


	dim MCBATID, MCBARID, MODE
	MCBARID = "NEW"
	MCBATID = document.all.MCBATID.value
	MODE = document.body.getAttribute("ScreenMode")
	
	MCBranchAssignRuleSearchObj.Selected = false

	strURL = "MCBranchAssignRuleModal.asp?MCBATID=" & MCBATID & "&MCBARID=" & MCBARID & "&MODE=" & MODE 	
	showModalDialog  strURL,MCBranchAssignRuleSearchObj ,"center"
...
Sub Refresh
	MCBATID = document.all.MCBATID.value
	document.all.tags("IFRAME").item("DataFrame").src = "MCBranchAssignTypeDetailsData.asp?MCBATID=" & MCBATID
End Sub
...
<%If HasModifyPrivilege("FNSD_MC_BRANCH_ASSIGNMENT","") <> True Then  %>		
		MsgBox "You do not have the appropriate security privileges to edit managed care branch assignment rules.",0,"FNSNetDesigner"
		Exit Sub
<%End If %>		

	dim MCBARID, MCBATID
	MCBARID = GetSelectedMCBARID
	MCBATID = document.all.MCBATID.value
	...
	If MCBARID <> "" Then
		...
		If MCBranchAssignRuleSearchObj.Selected = true Then	Refresh
	Else
		MsgBox "Please select a managed care branch assignment rule to Edit.", 0, "FNSNet Designer"		
	End If
End Sub
...
<%If HasDeletePrivilege("FNSD_MC_BRANCH_ASSIGNMENT","") <> True Then  %>		
		MsgBox "You do not have the appropriate security privileges to delete managed care branch assignment rules.",0,"FNSNetDesigner"
		Exit Sub
<%End If %>		
	...
	If MCBARID <> "" Then
		sResult = sResult &  MCBARID
		document.all.TxtSaveData.Value = sResult
		document.all.TxtAction.Value = "DELETE"
		FrmDetails.action = "MCBranchAssignRuleSave.asp"
		FrmDetails.method = "POST"
		FrmDetails.target = "hiddenPage"	
		FrmDetails.submit
		Refresh
	Else
		MsgBox "Please select a managed care branch assignment rule to Remove.", 0, "FNSNet Designer"		
	End If
...
Function ExeCopy
	...
	FrmDetails.action = "MCBranchAssignTypeCopy.asp"
	FrmDetails.method = "GET"
	FrmDetails.target = "hiddenPage"	
	FrmDetails.submit
...
'	If CStr(document.body.getAttribute("ScreenDirty")) = "YES" Then
		...
		document.all.TxtSaveData.Value = sResult
		FrmDetails.action = "MCBranchAssignTypeSave.asp"
		FrmDetails.method = "POST"
		FrmDetails.target = "hiddenPage"	
		FrmDetails.submit
...
<tr><td CLASS="GrpLabel" WIDTH="134" HEIGHT="10"><nobr>&nbsp;» Managed Care Branch Assignment Type Details&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<IMG SRC="../Images/HelpIcon2.gif" STYLE="CURSOR:HAND" align=absmiddle title=Help OnClick='LaunchHelp("Welcome.htm")'></td>
...
<form Name="FrmDetails" METHOD="POST" ACTION="MCBranchAssignTypeSave.asp" TARGET="hiddenPage">
<input TYPE="HIDDEN" NAME="TxtSaveData">
<input TYPE="HIDDEN" NAME="TxtAction">

<% 'need to maintain these values in order to post back to the search tab %>
<input type="hidden" name="SearchMCBATID" value="<%=Request.QueryString("SearchMCBATID")%>">
...
If MCBATID <> "" Then
	If MCBATID <> "NEW" Then
		...
		SQLST = "SELECT MC_BRANCHASSIGNMENTTYPE.*, RULES.*, ACCOUNT_HIERARCHY_STEP.NAME FROM " &_
				"MC_BRANCHASSIGNMENTTYPE, RULES, ACCOUNT_HIERARCHY_STEP WHERE MC_BRANCHASSIGNMENTTYPE.RULE_ID = RULES.RULE_ID(+) AND " &_
				"MC_BRANCHASSIGNMENTTYPE.ACCNT_HRCY_STEP_ID = ACCOUNT_HIERARCHY_STEP.ACCNT_HRCY_STEP_ID(+) AND " &_
				"MC_BRANCHASSIGNMENTTYPE_ID = " & MCBATID
...
<tr></tr>
<tr></tr>
<tr><td colspan=2>Managed Care Branch Assignment Type ID:&nbsp;<span id="spanMCBATID"><%=Request.QueryString("MCBATID")%></span></td></tr>
<tr>
...
<table WIDTH="100%" CELLPADDING="0" CELLSPACING="0">
<tr><td colspan="2" HEIGHT="4"></td></tr>
<tr><td CLASS="GrpLabel" WIDTH="134" HEIGHT="10"><nobr>&nbsp;» Managed Care Branch Assignment Rules</td>
<td HEIGHT="5" ALIGN="LEFT">
<table CELLPADDING="0" CELLSPACING="0" HEIGHT="100%">
<tr><td WIDTH="3" HEIGHT="4"></td><td WIDTH="300" HEIGHT="4"></td></tr>
<tr><td CLASS="GrpLabelDrk" WIDTH="3" HEIGHT="8" VALIGN="BOTTOM" ALIGN="LEFT"></td>
...
<iframe width=100% height=0 name="DataFrame" src="MCBranchAssignTypeDetailsData.asp?<%=Request.QueryString%>">
</fieldset>

<% Else %>

<div style="margin-top:170px;margin-left:170px" CLASS="LABEL">
No managed care branch assignment type selected.
</div>
