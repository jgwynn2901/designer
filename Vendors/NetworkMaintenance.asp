...
<%
	...
	If SELECTONLY <> "TRUE" Then
		If HasModifyPrivilege("FNSD_NETWORKS",SECURITYPRIV) <> True Then MODE = "RO"
	Else
		MODE ="RO"
	End If
	...
	If HasAddPrivilege("FNSD_NETWORKS",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
...
{
	NetObj.NetID = document.frames("TabFrame").document.frames.GetNID();
}

function IsCurrentSelectionValid()
{
	var NID = document.frames("TabFrame").document.frames.GetNID();
...
<%  if CONTAINERTYPE	= "MODAL" then %>
	NetObj = window.dialogArguments;
<%	end if %>
		
<%	if bShowSearchTab = true then %> 
	AddTab("Search",120, "NetworkSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "NetworkDetails-f.asp?MODE=<%=MODE%>&NID=<%=Request.QueryString("NID")%>",1);
<%	end if %>

<%	If (Request.QueryString("NID") = "NEW" Or Request.QueryString("NID") = "") And bShowSearchTab <> false Then %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "NetworkDetails-f.asp?MODE=<%=MODE%>&NID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
