...
<%
	...
	if CStr(Request.QueryString("COPYPASTE")) <> "" then COPYPASTE = CStr(Request.QueryString("COPYPASTE"))

	If SELECTONLY <> "TRUE" Then
		If HasModifyPrivilege("FNSD_ATTRIBUTE",SECURITYPRIV) <> True Then MODE = "RO"
	Else
		MODE ="RO"
	End If
	...
	Select Case COPYPASTE
		Case "TRUE"
			bNewButtonName = "Copy"
			bNewButtonWidth = " STYLE='WIDTH:120' "
		Case "FALSE"
	End Select


	If HasAddPrivilege("FNSD_ATTRIBUTE",SECURITYPRIV) <> True Then
		bShowNew = false
		bShowCopy = false
	End If
	
	If HasDeletePrivilege("FNSD_ATTRIBUTE",SECURITYPRIV) <> True Then
		bShowDelete = false
	End If	
...
{
	AttributeSearchObj.AID = document.frames("TabFrame").document.frames.GetAID();
	AttributeSearchObj.AIDName = document.frames("TabFrame").document.frames.GetAIDName();
	AttributeSearchObj.AIDCaption = document.frames("TabFrame").document.frames.GetAIDCaption();
	AttributeSearchObj.AIDInputType = document.frames("TabFrame").document.frames.GetAIDInputType();
}

function IsCurrentSelectionValid()
{
	var AID = document.frames("TabFrame").document.frames.GetAID();
...
<%	if bShowSearchTab = true then %>
	AddTab("Search",120, "AttributeSearch-f.asp?MODE=<%=MODE%>", 1);
<%	end if 
	if bShowDetailsTab = true then %>
	AddTab("Details", 120, "AttributeDetails-f.asp?MODE=<%=MODE%>&AID=<%=Request.QueryString("AID")%>",1);
<%	end if %>
...
function BtnNew_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		ChangeTabURL("Details", "AttributeDetails-f.asp?MODE=<%=MODE%>&AID=NEW");
		SetActiveTabViaGet("Details");
		ConfigureButtonsOnActivateTab("Details");
...
function BtnSelect_onclick()
{
	if (document.frames("TabFrame").document.readyState == "complete") 
	{
		...
		else	
		{
			...
			{
				UpdateCurrentObjFromScreen();
				AttributeSearchObj.Selected = true;
				window.close();
			}
		}
	}
...
<td CLASS="LABEL"><button CLASS="StdButton" <%= bNewButtonWidth %> NAME="BtnSelect" ACCESSKEY="T" LANGUAGE=javascript onclick="return BtnSelect_onclick()">
<% If bNewButtonName <> "" Then %>
Copy to Clipboard
<% Else %>
Selec<u>t</u>
<% End If %>
