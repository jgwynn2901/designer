...
if (ToolBar_Supported)
{
	...
	var cCRLF = String.fromCharCode(10) + String.fromCharCode(13);
...
//	var aCurMSColor  = new Array(3);
	...
	document.write("<STYLE TYPE=\"text/css\">" + cCRLF);
	document.write("<!-- $WEFT -- Created by: Roberto Agit (roberto.agit@concentramc.com) on 7/7/00 --" + cCRLF);
	document.write("@font-face {" + cCRLF);
    document.write("	font-family: Verdana;" + cCRLF);
    document.write("	font-style:  normal;" + cCRLF);
    document.write("	font-weight: 700;" + cCRLF);
    document.write("	src: url(http://cha0s14t/fnsdesigner/reports/billing/VERDANA0.eot);" + cCRLF);
	document.write("}" + cCRLF);
	document.write("-->" + cCRLF);
	document.write("</STYLE>" + cCRLF);
	document.write("<SPAN ID='StartMenu' STYLE='display:none;'></SPAN>");

	// Build toolbar template
	TBLStr  = "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR STYLE='height:22;vertical-align:middle'><!--ICP_MENU_TITLES--></TR></TABLE>";
	HTMLStr = 
		"<DIV ID='idToolbar'     STYLE='background-color:white;width:100%'>" +
		"<DIV ID='idRow3' STYLE='position:relative;height:25;width:100%'>" +
		"<DIV ID='idICPMenuPane' STYLE='position:absolute;top:0;left:0;height:25;color:white;background-color:black;' NOWRAP><!--ICP_MENU_TITLES--></DIV>" +
		"</DIV>" +
		"</DIV>" +
...
function addICPMenu(MenuIDStr, MenuDisplayStr, MenuHelpStr, MenuURLStr, cFuncName, cFrameName)
{ 	
	...
	if (addICPMenu.arguments.length > 4)
		...
		TargetStr = cFrameName;
	tempID = "ICP_" + MenuIDStr;
	addMenu(tempID, MenuDisplayStr, MenuHelpStr, MenuURLStr, TargetStr, true, cFuncName); 
	LastICPMenu = tempID;
}
...
function addMenu(MenuIDStr, MenuDisplayStr, MenuHelpStr, MenuURLStr, TargetStr, bICPMenu, cFuncName)
{
	...
	MenuStr = newLineChar;
	if (bICPMenu) 
		MenuStr += "<TD STYLE='" + cStyle + "VALIGN='MIDDLE'; ID='AM_" + MenuIDStr + "' NOWRAP>";
	else
	{
