...
Module ChubSummaryFixed

    Sub getChubClaimsFixedPricing(ByRef oRpt As ChubBillingSummary, ByVal cAHS_ID As String, ByVal cClient As String, ByVal cReportStartDate As String, ByVal cReportEndDate As String)
        Dim cSQL As String
        ...
        Dim nTotalClaimsMails As Integer
        ...
        cSQL = "Select LOB_CD, count(*) as nTotal From CALL, CALL_CALLER " & _
              ...
              "AND CLIENT_HRCY_STEP_ID = " & cAHS_ID & _
              " AND CALL_CALLER.CALL_ID = CALL.CALL_ID + 0 " & _
              "AND SUBSTR(CALL_CALLER.CALLER_TYPE, 1, 1) = 'M' " & _
                 "Group by LOB_CD"
        oCmd.CommandText = cSQL
        oReader = oCmd.ExecuteReader()
        nTotalClaimsMails = 0
        ...
        If oReader.HasRows Then
            Do
                ...
                Select Case oReader.GetValue(oReader.GetOrdinal("LOB_CD"))
                    ...
                    Case "PLI"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nPLIMails")
                    Case "PPR"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nPPRMails")
                    Case "CAU"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCAUMails")
                    Case "CLI"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCLIMails")
                    Case "CPR"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCPRMails")
                    Case "WOR"
                ...
                If Not lIsINFO Then
                    oParamFld.Text = CStr(oReader.GetValue(oReader.GetOrdinal("nTotal")))
                    nTotalClaimsMails = nTotalClaimsMails + CInt(oReader.GetValue(oReader.GetOrdinal("nTotal")))
                End If
            Loop Until Not oReader.Read
        End If
        oReader.Close()
        ...
        getTieredFees(oConn, oRpt, cAHS_ID, "M", nSERVICE_FEE, nTotalClaimsMails)
        oConn.Close()
        oConn = Nothing
    ...
    Private Sub getTieredFees(ByVal oConn As OracleConnection, _
                ByVal oRpt As ChubBillingSummary, _
