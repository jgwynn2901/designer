...
Module SRSBillingSummary

    Sub getSRSClaimsFixedPricing(ByRef oRpt As SRSBillingSummaryFixed, ByVal cAHS_ID As String, ByVal cClient As String, ByVal cReportStartDate As String, ByVal cReportEndDate As String, ByVal lIsCCE As Boolean)
        Dim cSQL As String
        ...
        '
        '   get call claims
        cSQL = "Select CALL.LOB_CD, count(*) as nTotal From CALL, CALL_CALLER,CALL_CLAIM  " & _
                "Where CALL_START_TIME >= TO_DATE('" & cStart & "') " & _
             "AND CALL_START_TIME < TO_DATE('" & cEnd & "') " & _
             "AND CALL.STATUS = 'COMPLETED' " & _
             "AND CALL_CALLER.CALL_ID = CALL.CALL_ID + 0 " & _
             "AND CALL_CLAIM.CALL_ID  = CALL.CALL_ID " & _
             "AND NVL(CALL_CLAIM.LOSS_ENTRY_TYPE,'0') <>'INT'" & _
             "AND (CALL_CALLER.CALLER_TYPE IS NULL OR (SUBSTR(CALL_CALLER.CALLER_TYPE, 1, 3) NOT IN ('EML','FAX'))) "
        If lIsCCE Then
            cSQL = cSQL & "AND CALL.ACCNT_HRCY_STEP_ID = " & cAHS_ID
        Else
            cSQL = cSQL & "AND CALL.CLIENT_HRCY_STEP_ID = " & cAHS_ID
        End If
        cSQL = cSQL & " Group by CALL.LOB_CD"
        ...
        cSQL = "Select LOB_CD, count(*) as nTotal From CALL, CALL_CALLER,CALL_CLAIM " & _
             ...
             "AND CALL_CALLER.CALL_ID = CALL.CALL_ID + 0 " & _
             "AND CALL_CLAIM.CALL_ID = CALL.CALL_ID " & _
             "AND CALL_CLAIM.LOSS_ENTRY_TYPE<>'INT' " & _
            ...
            Do
                ...
                Select Case CType(oReader.GetValue(oReader.GetOrdinal("LOB_CD")), String)
                    ...
                    Case "CLI"
                        ...
                        getFees(oConn, oRpt, cAHS_ID, "CLI", "F", nFAX_FEE, "nCLIPriceF")
                    Case "CPR"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCPRFaxes")
                        getFees(oConn, oRpt, cAHS_ID, "CPR", "F", nFAX_FEE, "nCPRPriceF")
                    Case "WOR"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nWORFaxes")
                        getFees(oConn, oRpt, cAHS_ID, "WOR", "F", nFAX_FEE, "nWORPriceF")
                    Case "CRI"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCRIFaxes")
                        getFees(oConn, oRpt, cAHS_ID, "CRI", "F", nFAX_FEE, "nCRIPriceF")
                    Case "ALI"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCRIFaxes")
                        getFees(oConn, oRpt, cAHS_ID, "ALI", "F", nFAX_FEE, "nCRIPriceF")
                    Case "INF"
                        lIsINFO = True
                    Case Else
                        oParamFld = Nothing '   force an error if not found
        ...
        cSQL = "Select LOB_CD, count(*) as nTotal From CALL, CALL_CALLER,CALL_CLAIM " & _
             ...
             "AND CALL_CLAIM.CALL_ID = CALL.CALL_ID  " & _
             "AND CALL_CLAIM.LOSS_ENTRY_TYPE<>'INT' " & _
             "AND SUBSTR(CALL_CALLER.CALLER_TYPE, 1, 3) = 'EML' "
        ...
        If oReader.HasRows Then
            Do
                ...
                Select Case CType(oReader.GetValue(oReader.GetOrdinal("LOB_CD")), String)
                    ...
                    Case "ALI"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCRIEmails")
                        getFees(oConn, oRpt, cAHS_ID, "ALI", "E", nSERVICE_FEE, "nCRIPriceE")
                    Case "INF"
                        lIsINFO = True
                    Case Else
                        oParamFld = Nothing '   force an error if not found
        ...
        'get internet claims
        cSQL = "Select LOB_CD, count(*) as nTotal From CALL,CALL_CLAIM,CALL_CALLER " & _
                "Where CALL_START_TIME >= TO_DATE('" & cStart & "') " & _
             "AND CALL_START_TIME < TO_DATE('" & cEnd & "') " & _
             "AND CALL.STATUS = 'COMPLETED' " & _
             "AND CALL_CALLER.CALL_ID = CALL.CALL_ID + 0 " & _
             "AND CALL_CLAIM.CALL_ID = CALL.CALL_ID  " & _
             "AND CALL_CLAIM.LOSS_ENTRY_TYPE = 'INT'"
    ...
    Sub getFees(ByVal oConn As OracleConnection, _
                ByVal oRpt As SRSBillingSummaryFixed, _
