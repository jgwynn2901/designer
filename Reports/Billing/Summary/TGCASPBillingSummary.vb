...
'--------------------------------------------------------------------------------------------------------------------*/
'WORK REQUEST       : JCER-0466 updated [TDS/SOW Document # if Exists :]
'FNS DESIGNER/
'Client			    : TOW
'Script Date        : 05/15/2013	By: Syed Waqas Ahmed Shah
'Requirement		: JCER-0466 Billing Report should include Internet With System Rekeying by IFN
'*/
'---------------------------------------------------------------------------------------------------------------------->
...
Module TGCASPBillingSummary

    Sub getClaimsTGCASP(ByRef oRpt As TGCASPBillingSummaryFixed, ByVal cAHS_ID As String, ByVal cClient As String, ByVal cReportStartDate As String, ByVal cReportEndDate As String)
        Dim cSQL As String
        ...
        SqlStrBuilder.Append(" From CALL,call_claim,USERS ")
        SqlStrBuilder.Append(" Where ")
        SqlStrBuilder.Append(" CALL.CALL_ID  = call_claim.CALL_ID(+) ")
        SqlStrBuilder.Append(" AND CALL.USER_ID = USERS.USER_ID ")
        ...
        SqlStrBuilder.Append(" AND USERS.SITE_ID = 1 ")
        SqlStrBuilder.Append(" AND call.client_hrcy_step_id = " & cAHS_ID)
        ...
        'SqlStrBuilder.Append(" where ACCNT_HRCY_STEP_ID=" & cAHS_ID & ")")
        ...
        SqlStrBuilder.Append(" From CALL, CALL_CALLER ,CALL_CLAIM,USERS ")
        SqlStrBuilder.Append(" Where ")
        ...
        '   get internet claims
        SqlStrBuilder = New System.Text.StringBuilder("")
        SqlStrBuilder.Append(" Select LOB_CD, count(*) as nTotal From CALL, CALL_CALLER ,CALL_CLAIM, USERS ")
        ...
        End If
        oReader.Close()
        '-- JCER-0466  Billing Report should include Internet With System Rekeying by IFN
        '   get web claims
        SqlStrBuilder = New System.Text.StringBuilder("")
        SqlStrBuilder.Append(" Select LOB_CD, count(*) as nTotal From CALL, CALL_CALLER ,CALL_CLAIM, USERS ")
        SqlStrBuilder.Append(" Where CALL_START_TIME >= TO_DATE('" & cStart & "') ")
        ...
        SqlStrBuilder.Append(" AND CALL_CALLER.CALL_ID = CALL.CALL_ID + 0 ")
        SqlStrBuilder.Append(" AND CALL.CALL_ID = CALL_CLAIM.CALL_ID ")
        SqlStrBuilder.Append(" AND CALL_CLAIM.CLAIM_TYPE = 'WEB' ")
        SqlStrBuilder.Append(" AND call.client_hrcy_step_id = " & cAHS_ID)
        SqlStrBuilder.Append("  Group by LOB_CD")
        strSQL = SqlStrBuilder.ToString
        oCmd.CommandText = SqlStrBuilder.ToString
        ...
        If oReader.HasRows Then
            Do
                ...
                Select Case CType(oReader.GetValue(oReader.GetOrdinal("LOB_CD")), String)
                    Case "PAU"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nPAUWeb")
                        getFees(oConn, oRpt, cAHS_ID, "PAU", "W", nSERVICE_FEE, "nPAUPriceW")
                    Case "PLI"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nPLIWeb")
                        getFees(oConn, oRpt, cAHS_ID, "PLI", "W", nSERVICE_FEE, "nPLIPriceW")
                    Case "PPR"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nPPRWeb")
                        getFees(oConn, oRpt, cAHS_ID, "PPR", "W", nSERVICE_FEE, "nPPRPriceW")
                    Case "CAU"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCAUWeb")
                        getFees(oConn, oRpt, cAHS_ID, "CAU", "W", nSERVICE_FEE, "nCAUPriceW")
                    Case "CLI"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCLIWeb")
                        getFees(oConn, oRpt, cAHS_ID, "CLI", "W", nSERVICE_FEE, "nCLIPriceW")
                    Case "CPR"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nCPRWeb")
                        getFees(oConn, oRpt, cAHS_ID, "CPR", "W", nSERVICE_FEE, "nCPRPriceW")
                    Case "WOR"
                        oParamFld = oRpt.DataDefinition.FormulaFields.Item("nWORWeb")
                        getFees(oConn, oRpt, cAHS_ID, "WOR", "W", nSERVICE_FEE, "nWORPriceW")
                    Case "INF"
                        lIsINFO = True
                    Case Else
                        oParamFld = Nothing '   force an error if not found
        ...
        '   get temped claims
        SqlStrBuilder = New System.Text.StringBuilder("")
        SqlStrBuilder.Append(" Select count(*) as nTotal From CALL, CALL_CLAIM, USERS ")
        SqlStrBuilder.Append(" Where CALL_START_TIME >= TO_DATE('" & cStart & "') ")
        SqlStrBuilder.Append(" AND CALL_START_TIME < TO_DATE('" & cEnd & "') ")
        SqlStrBuilder.Append(" AND STATUS = 'COMPLETED' ")
        SqlStrBuilder.Append(" AND CALL.CALL_ID = CALL_CLAIM.CALL_ID ")
        ...
        '   get escalations
        SqlStrBuilder = New System.Text.StringBuilder("")
        SqlStrBuilder.Append(" Select count(*) as nTotal From CALL, ESCALATION_OUTCOME, USERS ")
        SqlStrBuilder.Append(" Where CALL_START_TIME >= TO_DATE('" & cStart & "') ")
        SqlStrBuilder.Append(" AND CALL_START_TIME < TO_DATE('" & cEnd & "') ")
        ...
        SqlStrBuilder.Append(" From ROUTING_PLAN RP, CALL, USERS ")
    ...
    Sub getFees(ByVal oConn As OracleConnection, _
                ByVal oRpt As TGCASPBillingSummaryFixed, _
        ...
        cSQL = "Select FEE_AMOUNT From FEE, CALL, USERS " & _
                " Where(FEE.ACCNT_HRCY_STEP_ID = " & cAHS_ID & ") " & _
                " AND CALL.ACCNT_HRCY_STEP_ID = FEE.ACCNT_HRCY_STEP_ID " & _
                " AND CALL.USER_ID = USERS.USER_ID " & _
                " AND USERS.SITE_ID = 1 " & _
                " AND CALL_TYPE = '" & cCallType & "'" & _
                " AND FEE.LOB_CD = '" & cLOB & "' " & _
    ...
    Sub getProcessingFees(ByVal oConn As OracleConnection, _
        ...
        cSQL = "Select FEE_AMOUNT From FEE, CALL, USERS" & _
                ...
                " AND CALL.USER_ID = USERS.USER_ID " & _
                " AND USERS.SITE_ID = 1 " & _
                " AND FEE.FEE_TYPE_ID = " & nFeeTypeId
        oCmd.Connection = oConn
        oCmd.CommandText = cSQL
        oReader = oCmd.ExecuteReader()
        oReader.Read()
    ...
    Sub getFaxDescription(ByVal oConn As OracleConnection, _
        ...
        cSQL = "Select DESCRIPTION From FEE, CALL, USERS" & _
                " Where FEE.ACCNT_HRCY_STEP_ID = " & cAHS_ID & _
                " AND CALL.ACCNT_HRCY_STEP_ID = FEE.ACCNT_HRCY_STEP_ID " & _
                " AND CALL.USER_ID = USERS.USER_ID " & _
                " AND USERS.SITE_ID = 1 " & _
                " AND FEE.FEE_TYPE_ID = 2"
        oCmd.Connection = oConn
        oCmd.CommandText = cSQL
        oReader = oCmd.ExecuteReader()
        oReader.Read()
    ...
    Function getFaxFreeCount(ByVal oConn As OracleConnection, _
        ...
        getFaxFreeCount = 0
        cSQL = "Select FREE_COUNT From FEE, CALL, USERS" & _
                " Where FEE.ACCNT_HRCY_STEP_ID = " & cAHS_ID & _
                " AND CALL.ACCNT_HRCY_STEP_ID = FEE.ACCNT_HRCY_STEP_ID " & _
                " AND CALL.USER_ID = USERS.USER_ID " & _
                " AND USERS.SITE_ID = 1 " & _
    ...
    Function getFreeINFOPercent(ByVal oConn As OracleConnection, _
        ...
        cSQL = "Select FREE_PERCENTAGE From FEE, CALL, USERS" & _
                " Where FEE.ACCNT_HRCY_STEP_ID = " & cAHS_ID & _
                " AND CALL.ACCNT_HRCY_STEP_ID = FEE.ACCNT_HRCY_STEP_ID " & _
                " AND CALL.USER_ID = USERS.USER_ID " & _
                " AND USERS.SITE_ID = 1 " & _
