...
Module summaryPrudent

    Sub getClaimsFixedPrudent(ByRef oRpt As PrudenBillingSummary, ByVal cAHS_ID As String, ByVal cClient As String, ByVal cReportStartDate As String, ByVal cReportEndDate As String)
        Dim cSQL As String
        ...
        SqlStrBuilder.Append(" From CALL Where CALL_START_TIME >= TO_DATE('" & cStart & "') ")
        ...
        SqlStrBuilder.Append(" AND ACCNT_HRCY_STEP_ID = " & cAHS_ID)
        ...
        '   get INF claims
        SqlStrBuilder = New System.Text.StringBuilder("")
        SqlStrBuilder.Append(" Select count(*) as nTotal From CALL ")
            ...
            Do
                ...
                Select Case oReader.GetValue(oReader.GetOrdinal("LOB_CD"))
                    Case "PAU"
                        ...
                        getFeesFinaceProperty(oConn, oRpt, cStart, cEnd, cAHS_ID, "PAU", "C", nSERVICE_FEE, "nPAUCallDollars")
        ...
        '   get faxed claims
        SqlStrBuilder = New System.Text.StringBuilder("")
        SqlStrBuilder.Append(" Select LOB_CD, count(*) as nTotal From CALL, CALL_CALLER ")
        SqlStrBuilder.Append("  Where CALL_START_TIME >= TO_DATE('" & cStart & "') ")
        SqlStrBuilder.Append(" AND CALL_START_TIME < TO_DATE('" & cEnd & "') ")
        SqlStrBuilder.Append(" AND STATUS = 'COMPLETED' ")
        SqlStrBuilder.Append(" AND CALL_CALLER.CALL_ID = CALL.CALL_ID + 0 ")
    ...
    Sub getFees(ByVal oConn As OracleConnection, _
                ByVal oRpt As PrudenBillingSummary, _
    ...
    End Function
    Sub getFeesFinaceProperty(ByVal oConn As OracleConnection, _
                ByVal oRpt As PrudenBillingSummary, _
                ByVal cStart As String, _
                ByVal cEnd As String, _
                ByVal cAHS_ID As String, _
                ByVal cLOB As String, _
                ByVal cCallType As String, _
                ByVal nFeeTypeId As Integer, _
        ...
        Dim oCmd As New OracleCommand
        Dim oReader As OracleDataReader
        Dim oReaderFee As OracleDataReader
        oCmd.Connection = oConn
        Dim SqlStrBuilder As System.Text.StringBuilder
        '********************************************************
        SqlStrBuilder = New System.Text.StringBuilder("")
        SqlStrBuilder.Append(" Select LOB_CD,CALL_CLAIM.GLASS_DAMAGE_FLG , count(*) as nTotal ")
        SqlStrBuilder.Append(" From CALL, CALL_CALLER ,CALL_CLAIM ")
        SqlStrBuilder.Append(" Where CALL_START_TIME >= TO_DATE('" & cStart & "') ")
        SqlStrBuilder.Append(" AND CALL_START_TIME < TO_DATE('" & cEnd & "') ")
        SqlStrBuilder.Append(" AND STATUS = 'COMPLETED' ")
        ...
        SqlStrBuilder.Append(" AND (CALL_CLAIM.CLAIM_TYPE IS NULL OR (SUBSTR(CALL_CLAIM.CLAIM_TYPE, 1, 1) <> 'N'))")
        SqlStrBuilder.Append(" AND ACCNT_HRCY_STEP_ID = " & cAHS_ID)
        SqlStrBuilder.Append(" AND CALL_CLAIM.GLASS_DAMAGE_FLG  is not null ")
        SqlStrBuilder.Append(" AND LOB_CD <> 'INF'")
        SqlStrBuilder.Append(" Group by LOB_CD,CALL_CLAIM.GLASS_DAMAGE_FLG")
        ' oCmd.Connection = oConn
        oCmd.CommandText = SqlStrBuilder.ToString
        oReader = oCmd.ExecuteReader()
        oReader.Read()
        Dim strFlagGlass As String
        Dim iCount As Integer
        Dim strCallPAUPsice As Integer
        strCallPAUPsice = 0
        If oReader.HasRows Then
            Do
                strFlagGlass = "" & CStr(oReader.GetValue(oReader.GetOrdinal("GLASS_DAMAGE_FLG")))

                cSQL = "Select FEE_AMOUNT From FEE Where ACCNT_HRCY_STEP_ID = " & cAHS_ID & _
                    " AND CALL_TYPE = '" & cCallType & "'" & _
                    " AND LOB_CD = '" & cLOB & "' " & _
                    " AND FEE_TYPE_ID = " & nFeeTypeId & _
                    " AND REASON_CODE = '" & strFlagGlass & "'"
                'oCmd.Connection = oConn
                oCmd.CommandText = cSQL
                oReaderFee = oCmd.ExecuteReader(CommandBehavior.SingleResult)
                oReaderFee.Read()
                Dim strFeeAmmount As Integer
                Dim SummAll As Integer
                If oReaderFee.HasRows Then

                    strFeeAmmount = CInt(oReaderFee.GetValue(oReaderFee.GetOrdinal("FEE_AMOUNT")))
                    iCount = CInt(oReader.GetValue(oReader.GetOrdinal("nTotal")))
                    strCallPAUPsice = iCount * strFeeAmmount
                    SummAll = SummAll + strCallPAUPsice

                End If

                'oParamFld = oRpt.DataDefinition.FormulaFields.Item(cFormulaName)+
                strCallPAUPsice = SummAll


                oReaderFee.Close()
            Loop Until Not oReader.Read
            oParamFld = oRpt.DataDefinition.FormulaFields.Item(cFormulaName)
            oParamFld.Text = CStr(strCallPAUPsice)
