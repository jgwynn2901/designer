...
Module summaryMGCWithConfirmation

    Sub getMGCClaimsWithConfirmation(ByRef oRpt As BillingSummaryTiered, ByVal cAHS_ID As String, ByVal cClient As String, ByVal cReportStartDate As String, ByVal cReportEndDate As String)
        Dim cSQL As String
        ...
        Dim nTotalClaimsNOConfirm As Integer
        Dim nTotalClaimsConfirm As Integer
        ...
        '   get total number of claims w/confirmation (excluding INF calls)
        cSQL = "Select COUNT(CALL.CALL_ID) AS totalCalls " & _
                "From CALL, CALL_ASI CAS, X_CALL_ASI XCAS Where CALL_START_TIME >= TO_DATE('" & cStart & "') " & _
                "AND CALL_START_TIME < TO_DATE('" & cEnd & "') " & _
                "AND STATUS = 'COMPLETED' " & _
                "AND LOB_CD <> 'INF' " & _
                "AND CAS.CALL_ASI_ID = XCAS.CALL_ASI_ID " & _
                "AND XCAS.FIELD = 'IRC_CONFIRMATION_FLG' " & _
                "AND CAS.CALL_ID = CALL.CALL_ID " & _
                "AND XCAS.VALUE = 'Y' " & _
                "AND CLIENT_HRCY_STEP_ID = 103"
        ...
        nTotalClaimsConfirm = 0
        oReader.Read()
        If oReader.HasRows Then
            nTotalClaimsConfirm = CInt(oReader.GetValue(oReader.GetOrdinal("totalCalls")))
        End If
        oReader.Close()
        '   get INF claims
        cSQL = "Select COUNT(CALL.CALL_ID) AS nTotal " & _
                "From CALL, CALL_ASI CAS, X_CALL_ASI XCAS Where CALL_START_TIME >= TO_DATE('" & cStart & "') " & _
                "AND CALL_START_TIME < TO_DATE('" & cEnd & "') " & _
                "AND STATUS = 'COMPLETED' " & _
                "AND LOB_CD = 'INF' " & _
        ...
        'getINFFees(oConn, oRpt, cAHS_ID, "INF", "I", nSERVICE_FEE, "nINFPrice")
        ...
        '
        '   get call claims
        cSQL = "Select LOB_CD, COUNT(CALL.CALL_ID) AS nTotal " & _
                "From CALL, CALL_CALLER, CALL_ASI CAS, X_CALL_ASI XCAS Where CALL_START_TIME >= TO_DATE('" & cStart & "') " & _
                "AND CALL_START_TIME < TO_DATE('" & cEnd & "') " & _
                "AND STATUS = 'COMPLETED' " & _
        ...
        oReader.Close()
        '   get temped claims
        cSQL = "Select count(*) as nTotal From CALL, CALL_CLAIM, CALL_ASI CAS, X_CALL_ASI XCAS  " & _
        ...
        oReader.Close()
        '   get escalations
        cSQL = "Select count(*) as nTotal From CALL, ESCALATION_OUTCOME, CALL_ASI CAS, X_CALL_ASI XCAS " & _
                "Where CALL_START_TIME >= TO_DATE('" & cStart & "') " & _
                "AND CALL_START_TIME < TO_DATE('" & cEnd & "') " & _
        ...
        cSQL = "Select COUNT(TOC.TRANSMISSION_OUTCOME_ID) AS transmissionCount " & _
          ...
          "AND TOC.CALL_ID IN (Select DISTINCT CALL.CALL_ID From CALL, CALL_ASI CAS, X_CALL_ASI XCAS " & _
          "Where CALL_START_TIME >= TO_DATE('" & cStart & "') " & _
          "AND CALL_START_TIME < TO_DATE('" & cEnd & "') " & _
          "AND STATUS = 'COMPLETED' " & _
          "AND CAS.CALL_ASI_ID = XCAS.CALL_ASI_ID " & _
        ...
        cSQL = "Select Sum(PAGE_COUNT) AS pageCount From TRANSMISSION_OUTCOME_STEP " & _
            ...
            "From ROUTING_PLAN RP, CALL, CALL_ASI CAS, X_CALL_ASI XCAS " & _
            "Where CLIENT_HRCY_STEP_ID = 103 AND CALL_START_TIME >= TO_DATE('" & cStart & "') " & _
            "AND CALL_START_TIME < TO_DATE('" & cEnd & "') " & _
            "AND CAS.CALL_ASI_ID = XCAS.CALL_ASI_ID " & _
            "AND XCAS.FIELD = 'IRC_CONFIRMATION_FLG' " & _
            "AND CAS.CALL_ID = CALL.CALL_ID " & _
        ...
        'If Not oRS.EOF Then
        'nTotalFaxedPages = CInt(oRS.Fields("pageCount").Value)
        'Else
        'nTotalFaxedPages = 0
        'End If
        'oRS.Close()
    ...
    Private Sub getINFFees(ByVal oConn As OracleConnection, _
                ByVal oRpt As BillingSummaryTiered, _
    ...
    Private Sub getTieredFees(ByVal oConn As OracleConnection, _
        ...
        cTierTO = "nTier~TO"
        x = 1
        cSQL = "Select * From FEE Where ACCNT_HRCY_STEP_ID = " & cAHS_ID & _
                " AND CALL_TYPE = '" & cCallType & "'" & _
