...
'Template
Set NewObj = objCol.AddItemTemplate("OVERRIDE", "",  "", 0, 0, 15, 4, "ARIAL", 9,false ,false ,false ,false ,false , 1, "", "MODIFY",  "", False	)
	NewObj.ItemBackColor = 16748574
	NewObj.ItemTextColor = 100
...
%>
	Set NewObj = objCol.AddItem ("OUTPUT_FLD_ID=<%= RS("OUTPUT_FLD_ID") %>", "<%= RS("NAME") %>",  "X", <%= RS("XPOS") %>, <%= RS("YPOS") %>, <%= RS("WIDTH") %>, <%= RS("HEIGHT") %>, "<%= RS("FONT_NAME") %>", <%= RS("FONT_SIZE") %>, <%= MULTILINE_FLG %>, <%= STRIKEOUT_FLG %>, <%= UNDERLINE_FLG %>, <%= ITALIC_FLG %>,<%=  BOLD_FLG %>, 0, "FIELD", "MODIFY",  "<%= VB_MAPPING %>", False	)
	NewObj.SetExtraProperty "OUTPUT_FLD_ID", true, "<%= RS("OUTPUT_FLD_ID") %>"
	NewObj.readonly=true
	NewObj.SetExtraProperty "BMP", true, "<%= RS("BMP") %>"
<%
	If IDList = "" Then
		IDList = IDList & RS("OUTPUT_FLD_ID")
	Else
		IDList = IDList & ", " & RS("OUTPUT_FLD_ID")
	End If
	RS.MoveNext
	...
	If IDList <> "" Then
	SQLOVER = ""
	SQLOVER = SQLOVER & "SELECT * FROM OUTPUT_MAPPING WHERE OUTPUT_FLD_ID IN (" & IDList & ") AND " 
	SQLOVER = SQLOVER & " ACCNT_HRCY_STEP_ID=" & Request.QueryString("AHSID") & " ORDER BY NAME "
	Set RSOver = Conn.Execute(SQLOVER)
	Do While NOT RSOver.EOF
	MULTILINE_FLG = ConBool(RSOver("MULTILINE_FLG"))
	UNDERLINE_FLG = ConBool(RSOver("UNDERLINE_FLG"))
	BOLD_FLG = ConBool(RSOver("BOLD_FLG"))
	ITALIC_FLG = ConBool(RSOver("ITALIC_FLG"))
	STRIKEOUT_FLG = ConBool(RSOver("STRIKEOUT_FLG"))
	VB_MAPPING = ReplaceStr( ReplaceStr (RSOver("MAPPING"), """" , """"""), VBCrlf, "")
	If IsNull(RSOver("FONT_SIZE")) Then
		FONT_SIZE = 0
	Else
		FONT_SIZE = RSOver("FONT_SIZE")
	End If
	%>
	
	Set NewObj = objCol.AddItem ("<%= RSOver("OUTPUT_MAPPING_ID") %>", "<%= RSOver("NAME") %>",  "X", <%= RSOver("XPOS") %>, <%= RSOver("YPOS") %>, <%= RSOver("WIDTH") %>, <%= RSOver("HEIGHT") %>, "<%= RSOver("FONT_NAME") %>", <%= FONT_SIZE %>, <%= MULTILINE_FLG %>, <%= STRIKEOUT_FLG %>, <%= UNDERLINE_FLG %>, <%= ITALIC_FLG %>,<%=  BOLD_FLG %>, 0, "OVERRIDE", "MODIFY",  "<%= VB_MAPPING %>", False	)
	<% If MODE = "RO" Then %>
	NewObj.readonly=true
	<% End If %>
	NewObj.SetExtraProperty "ACCNT_HRCY_STEP_ID", true, "<%= RSOver("ACCNT_HRCY_STEP_ID") %>"
	NewObj.SetExtraProperty "OUTPUT_FLD_ID", true, "<%= RSOver("OUTPUT_FLD_ID") %>"
	NewObj.SetExtraProperty "OUTPUT_MAPPING_ID", true, "<%= RSOver("OUTPUT_MAPPING_ID") %>"
	NewObj.SetExtraProperty "BMP", true, "<%= RSOver("BMP") %>"

<%
	RSOver.MoveNext
	Loop
	If RSOver.EOF AND NOT RSOver.BOF Then
		RSOver.MoveFirst
	End If
End If
	If Not RS.EOF Then
	...
	Else
		SQL1 = ""
		SQL1 = SQL1 & "SELECT BACKGROUND_BMP FROM OUTPUT_PAGE WHERE OUTPUT_PAGE_ID = " & Request.QueryString("OPID")
...
LayoutCtl.SetContextMenu "OVERRIDE", "COMMAND|Properties...|400|SEPARATOR|0|0|COMMAND|Remove Override|401", "|" 
...
Parent.Frames("TOPAREA").document.all.Listattributes.selectedindex=0
LoadOverrideDropDown
Parent.Frames("TOPAREA").document.all.Listoverrides.selectedindex=0
...
objOption.value = "ALLOVERRIDES"
objOption.Text = "All Overrides"
...
Function LoadOverrideDropDown
If IsObject (Parent.Frames("TOPAREA").document.all.Listoverrides) = True Then
RemoveOptions Parent.Frames("TOPAREA").document.all.Listoverrides
Set objOption = Parent.Frames("TOPAREA").document.createElement("option")
Parent.Frames("TOPAREA").document.all.Listoverrides.options.add( objOption )	
Set objCol = LayoutCtl.PageItems
For each x in objCol
If x.Readonly = false Then
	...
	objOption.Text = x.AttributeKey
	Parent.Frames("TOPAREA").document.all.Listoverrides.options.add( objOption )
	Parent.Frames("TOPAREA").document.all.Listoverrides.style.backgroundColor="WHITE"
End If
Next
Else
	MsgBox "Override Load Drop Down Failed"
End If
End Function
...
If attrKey <> "" OR refID <> "" Then
		Set X =LayoutCtl.GetSelectedItem	
		If X.readonly = true Then
			Parent.Frames("TOPAREA").FilterOverrideList()
			LayoutCtl.SetContextMenu "OVERRIDE", "COMMAND|Properties...|400", "|" 	
			Parent.Frames("TOPAREA").document.all.ListAttributes.value = "OUTPUT_FLD_ID=" & x.GetExtraProperty("OUTPUT_FLD_ID")
		Else
			LayoutCtl.SetContextMenu "OVERRIDE", "COMMAND|Properties...|400|SEPARATOR|0|0|COMMAND|Remove Override|401", "|" 	
			Parent.Frames("TOPAREA").document.all.ListAttributes.value = "OUTPUT_FLD_ID=" & x.GetExtraProperty("OUTPUT_FLD_ID")
			...
			RemoveOptions Parent.Frames("TOPAREA").document.all.Listoverrides
			Set objOption = Parent.Frames("TOPAREA").document.createElement("option")
			objOption.value = X.GetExtraProperty("OUTPUT_MAPPING_ID")
			objOption.Text = X.AttributeKey
			Parent.Frames("TOPAREA").document.all.Listoverrides.options.add( objOption )
			Parent.Frames("TOPAREA").document.all.Listoverrides.selectedindex = 0
...
Sub DuplicateField
	...
	Set X=LayoutCtl.GetSelectedItem
	Set objCol = LayoutCtl.PageItems
	objCol.AddItem G_SequenceNumber, x.attributekey,  "X", x.xpos + x.height, x.ypos + x.height, x.width, x.height, x.fontname, x.fontpointsize, x.Multiline, x.strikeout,  x.underline,  x.italic, x.bold,  0, "TYPE", "NEW",  x.formatstring , False	
	Set objOption = Parent.Frames("TOPAREA").document.createElement("option")
...
<PARAM NAME="BackImageURL" VALUE="HTTP://<%= Request.servervariables("SERVER_NAME") %>/FNSDesigner/images/BGBMP/<%= BACKGROUND_BMP %>">
